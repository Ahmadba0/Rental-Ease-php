...
$stmt = $con->prepare("SELECT * FROM `userscars` WHERE `users_id` = ?");
$stmt->execute(array($users_id )) ;
$count = $stmt->rowCount();
$data = $stmt->fetchAll(PDO::FETCH_ASSOC);

$stmt1 = $con->prepare("SELECT * FROM `usersswim` WHERE `users_id` = ?");
$stmt1->execute(array($users_id )) ;
$count1 = $stmt1->rowCount();
$data1 = $stmt1->fetchAll(PDO::FETCH_ASSOC);

$stmt2 = $con->prepare("SELECT * FROM `userswidding` WHERE `users_id` = ?");
$stmt2->execute(array($users_id )) ;
$count2 = $stmt2->rowCount();
$data2 = $stmt2->fetchAll(PDO::FETCH_ASSOC);

$stmt3 = $con->prepare("SELECT * FROM `usersrealestate` WHERE `users_id` = ?");
